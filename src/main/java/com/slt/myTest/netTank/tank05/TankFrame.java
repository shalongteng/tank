...
public class TankFrame extends Frame {
    ...
    Random r = new Random();
    //tank 位置随机出现
    Tank myTank = new Tank(r.nextInt(GAME_WIDTH-40), r.nextInt(GAME_HEIGHT-40), Dir.DOWN, this, Group.GOOD);
...
//        this.setVisible(true);
    ...
    public void addTank(Tank t) {
        Optional<Tank> optional = tankList.stream().filter(p -> p.getId().equals(t.getId())).findAny();
    ...
    public Tank findByUUID(UUID id) {
        Optional<Tank> optional = tankList.stream().filter(p -> id.equals(p.getId())).findAny();
        if(optional.isPresent()){
            return optional.get();
        }
        return null;
